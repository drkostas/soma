...
function TabStrip({ tabs, value, onChange }: { tabs: { key: string; disabled?: boolean }[]; value: string; onChange: (k: string) => void }) {
  return (
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerClassName="gap-1.5 py-0.5 pr-4">
      {tabs.map((t) => {
        ...
        return (
          <Pressable key={t.key} disabled={t.disabled} onPress={() => onChange(t.key)} hitSlop={4} accessibilityRole="tab" accessibilityState={{ selected: active, disabled: !!t.disabled }} testID={`tab-${t.key.toLowerCase()}`}>
            <View className="rounded-full px-2.5 py-1" style={{ backgroundColor: active ? "#77c8d1" : "#152232", borderWidth: 1, borderColor: active ? "#77c8d1" : "#1a3040", opacity: t.disabled ? 0.4 : 1 }}>
              <Text variant="micro" style={{ color: active ? "#0a1720" : "#a0b4c0", fontWeight: active ? "700" : "500", fontSize: 12, lineHeight: 16 }}>{t.key}</Text>
            </View>
          </Pressable>
        );
      })}
