...
export function DeltaSimulator({ basePace, optimalPace, currentVdot, goalVdot }: DeltaSimulatorProps) {
  const [slider, setSlider] = useState(1.0);

  const delta = optimalPace - basePace;
  const scaledPace = basePace + delta * slider;
  const finalPace = Math.max(180, Math.min(600, scaledPace));

  const risk = slider > 1.3 ? "high" : slider > 1.0 ? "moderate" : "low";
      ...
      <CardContent className="space-y-3">
        ...
        <div className="flex items-center justify-between">
          <div className="text-xs text-muted-foreground">
            Aggression: <span className="font-mono font-medium">{slider.toFixed(2)}</span>
          </div>
        ...
        <div className="flex items-baseline gap-2 pt-1 border-t border-border/50">
          ...
          <span className="text-[10px] text-muted-foreground">
            ({slider === 0 ? "conservative" : slider === 1 ? "close gap" : slider > 1 ? "push beyond" : "partial close"})
          </span>
        </div>
      </CardContent>
    </Card>
