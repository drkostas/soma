...
import { materialise } from "../lib/capture-media";
import { uploadsDir } from "../lib/nutrition-agent";

const sql = getDb();
const rows = (await sql`SELECT id, byte_size FROM capture_media ORDER BY created_at DESC LIMIT 1`) as Array<{ id: string; byte_size: number }>;
