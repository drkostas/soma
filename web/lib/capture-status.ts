 ...
 *
 * The capture's state has always been in the database. What was missing is anything that says it
 * out loud, so a sentence posted from the phone looked identical to one never sent. These are the
 * only words used for it, shared by the web strip and the app strip so the two never drift.
 *
 * Kept free of React and of the database on purpose: the phrasing is the part worth testing, and
 * it is the part most likely to be argued about later.
 */
